(ns caiso.main
  "Kotodama WASM entrypoint for the Caiso clean-room actor (L4) — Clojure port.
  ...
  facts (`caiso.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"caiso.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "caiso-compat" :tier tier :entities entities} 200])
